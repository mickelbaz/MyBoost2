<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8" />
        <title>MyBoost - Le Site Web</title>
        <link rel="stylesheet" type="text/css" href="/MyBoost/Contenu/headerFooter.css">
    </head>
    <body>
<div id="bloc_page">
<header>

    <div id="titre_principal">
        <div class="logo">
                <img src="/MyBoost/Images/Logo4.png" alt="Logo de site web MyBoost"/>
                <h1>Un sport, un clic, une communauté!</h1>
            </ul>
        </div>
        <div class="inscription">
            <a href="/MyBoost/Controleurs/controleurInscription.php">Inscription</a>
        </div>

        <div class="connexion">
          <a href="/MyBoost/Controleurs/controleurConnexion.php">Connexion</a>
        </div>
    </div>
    <nav>
        <ul>
            <li class="nav-item"><a href="/MyBoost/index.php">Accueil</a></li>
            <li class="nav-item"><a href="#">Mon Compte</a></li>
            <li class="nav-item"><a href="#">Communauté</a>
               <!--<ul class="sub-nav">
                    <li class="sub-nav-item"><a href="#">Sports</a></li>
                    <li class="sub-nav-item"><a href="#">Annuaire</a></li>
                    <li class="sub-nav-item"><a href="#">Groupes</a></li>
                </ul>
            </li>-->
            <li class="nav-item"><a href="#">Forum</a></li>
            <li class="nav-item"><a href="#">À Propos</a></li>
        </ul>
    </nav>
    <div class="search">
        <input type="text" name="name" placeholder="rechercher">
    </div>
</header>
</div>
